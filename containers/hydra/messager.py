...
if file_exists:
    ...
    if hydradata is None:
        ...
        with open(hydradata) as jsonf:
            ...
            if int(buildstatus) is None:
                SLACKMESSAGE = "False build! Did not finish, maybe evaluation failed ??"

            elif int(buildstatus) == 0:
                SLACKMESSAGE = "OK build !!!"

            elif int(buildstatus) == 1:
                SLACKMESSAGE = "Failed build !!!"

            elif int(buildstatus) == 2:
                SLACKMESSAGE = "Dependency failed build !!!"

            elif int(buildstatus) == 3:
                SLACKMESSAGE = "Aborted build !!!"

            elif int(buildstatus) == 4:
                SLACKMESSAGE = "Cancelled by the user build !!!"

            elif int(buildstatus) == 5:
                SLACKMESSAGE = "Failed build by undefined reason !!!"

            elif int(buildstatus) == 6:
                SLACKMESSAGE = "Failed with output build !!!"

            elif int(buildstatus) == 7:
                SLACKMESSAGE = "Timedout build !!!"

            elif int(buildstatus) == 8:
                SLACKMESSAGE = "Failed build by undefined reason !!!"

            elif int(buildstatus) == 9:
                SLACKMESSAGE = "Aborted build !!!"

            elif int(buildstatus) == 10:
                SLACKMESSAGE = "Log size limit exceed failure build !!!"

            elif int(buildstatus) == 11:
                SLACKMESSAGE = "Output size limit exceed build !!!"

            else:
                SLACKMESSAGE = "Broken build for some undefined reason, maybe future error message ???"
